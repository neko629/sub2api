export default {
    accounts: {
      ...
      calibrateWindow: {
        menuItem: '校准用量窗口',
        title: '校准用量窗口',
        subtitle: '手动修正账号 {name} 的上游用量窗口刷新时刻。',
        oauthNotice: '这是 OAuth 账号：上游 Anthropic API 才是权威来源，本次校准会在下一次成功探测（最快 3 分钟）后被真实 resets_at 覆盖。仅在上游 usage 接口持续报错时才有意义。',
        setupTokenNotice: '这是 setup-token 账号：拿不到 profile scope，5 小时窗口只能靠响应头或本地预测，漂了无法自愈。校准会一直生效，直到下一条带真实 reset 响应头到达（那时以真实头为准，是预期行为）。',
        windowLabel: '窗口',
        window5h: '5h — 5 小时会话窗口',
        window7d: '7d — 7 天窗口',
        window7dOi: '7d Fable — Fable 专属 7 天窗口',
        resetsAtLabel: '下次刷新时间 (resets_at)',
        resetsAtHint: '必须晚于当前时刻，且不超过 {hours} 小时之后。',
        resetUsage: '同时清零该窗口的用量采样',
        referenceConfirm: '若该账号被设为「用量基准账号」，把刷新时刻改为\n{time}\n会同时平移所有用户的 5 小时 / 周限额窗口，并使当前窗口用量立即作废。\n\n确认继续？',
        success: '用量窗口已校准',
        failed: '校准失败',
        errors: {
          invalidTime: '请填写有效的时间',
          mustBeFuture: '刷新时间必须晚于当前时刻',
          outOfRange: '超出合理区间：最多只能设到 {hours} 小时之后',
        },
      },
      quotaLimit: '配额限制',
