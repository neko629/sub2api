export default {
    accounts: {
      ...
      calibrateWindow: {
        menuItem: 'Calibrate Usage Window',
        title: 'Calibrate Usage Window',
        subtitle: 'Manually correct the upstream usage window reset time for account {name}.',
        oauthNotice: 'This is an OAuth account: the upstream Anthropic API is authoritative, so this calibration will be overwritten by the real resets_at on the next successful probe (as soon as 3 minutes). It only helps while the upstream usage endpoint keeps failing.',
        setupTokenNotice: 'This is a setup-token account: without the profile scope the 5-hour window relies on response headers or a local prediction and cannot self-correct when it drifts. The calibration stays in effect until the next real reset header arrives (at which point the real value wins — that is intended).',
        windowLabel: 'Window',
        window5h: '5h — 5-hour session window',
        window7d: '7d — 7-day window',
        window7dOi: '7d Fable — Fable-only 7-day window',
        resetsAtLabel: 'Next reset time (resets_at)',
        resetsAtHint: 'Must be in the future and no more than {hours} hours from now.',
        resetUsage: "Also clear this window's usage sample",
        referenceConfirm: "If this account is configured as a usage reference account, changing the reset time to\n{time}\nwill also shift every user's 5-hour / weekly quota window and immediately void the current window's usage.\n\nContinue?",
        success: 'Usage window calibrated',
        failed: 'Calibration failed',
        errors: {
          invalidTime: 'Please enter a valid time',
          mustBeFuture: 'The reset time must be in the future',
          outOfRange: 'Out of range: at most {hours} hours from now',
        },
      },
