...
describe("admin SettingsView platform quota matrix", () => {
  ...
  it("加载后 form.default_platform_quotas 含全 5 平台，从嵌套 JSON 正确读取数值", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      default_platform_quotas: {
        anthropic: { five_hour: null, daily: 5, weekly: null, monthly: null },
        openai:    { five_hour: null, daily: null, weekly: 12.5, monthly: null },
  ...
  it("空输入（v-model.number 产出 \"\"）在提交时清洗为 null 而非空字符串", async () => {
    ...
    // 找到 anthropic 的 daily 输入框并清空（模拟用户删除值）。
    // 注意：「用量基准账号」表里也有 anthropic 行且带一个 number input，
    // 所以必须锁定「本行有 4 个 number input」的限额行，再取第 2 个（5h → 日）。
    const quotaRow = wrapper.findAll("tr").find((r) => {
      if (!r.text().includes("anthropic")) return false;
      return r.findAll('input[type="number"]').length === 4;
    });
    expect(quotaRow).toBeTruthy();
    const anthropicDailyInput = quotaRow!.findAll('input[type="number"]')[1];
